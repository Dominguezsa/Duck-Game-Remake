...
int main() try {
	...
	// texture
	Texture sprites(renderer, Surface(DATA_PATH "/imagenesDePatos.png")
			.SetColorKey(true, 0));
	...
	Texture gradient(renderer, SDL_PIXELFORMAT_RGB24, SDL_TEXTUREACCESS_STATIC, 1, 256);
	{
    ...
    bool looking_right = true; // player looking direction
    bool is_down = false; // player is down
	...
	while (1) {
		...
		while (SDL_PollEvent(&event)) {
			if (event.type == SDL_QUIT) {
				...
				case SDLK_ESCAPE: case SDLK_q:
					return 0;
				case SDLK_RIGHT: is_running = true; looking_right =true; break;
                case SDLK_LEFT: is_running = true; looking_right = false; break;
                case SDLK_DOWN: is_down = true; break;
				}
			} else if (event.type == SDL_KEYUP) {
				switch (event.key.keysym.sym) {
				case SDLK_RIGHT: is_running = false; break;
                case SDLK_LEFT: is_running = false; break;
                case SDLK_DOWN: is_down = false; break;
				}
			}
		}
		...
		if (looking_right){
            renderer.Copy(
                sprites,
                Rect(src_x, src_y, 64, 64),
                Rect((int)position, vcenter - 64, 64, 64),
                0.0 // don't rotate
            );
        } else {
            renderer.Copy(
		...
		renderer.Copy(
				sprites,
				Rect(src_x, src_y, 64, 64),
				Rect((int)position, vcenter, 64, 64),
				0.0,              // don't rotate
            ...
            renderer.Copy(
                ...
                0.0,              // don't rotate
                NullOpt,          // rotation center - not needed
                SDL_FLIP_VERTICAL | SDL_FLIP_HORIZONTAL // vertical flip
            );
        }
