...
private:
    ...
    /*
     * Construye el commonFiles pasándole directamente el file descriptor.
     * */
...
public:
     ...
     * el commonFiles activo y otra para el pasivo.
     *
     ...
     *
     * Más aun no tiene sentido pensar en copiar un commonFiles. Un commonFiles
     ...
     *
     * Si el commonFiles detecto que la conexión fue cerrada, la variable
     * `was_closed` es puesta a `true`, de otro modo sera `false`.
     *
     * Retorna 0 si se cerro el commonFiles,
     ...
     * Si hay un error se lanza una excepción.
     *
     * Si no hubo un error pero el commonFiles se cerro durante el envio/recibo
     ...
     *
     * En ambos casos, donde el commonFiles se cerró, `was_closed` es puesto a `true`.
     *
    ...
    /*
     * Acepta una conexión entrante y retorna un nuevo commonFiles
    ...
    /*
     * Cierra el commonFiles. El cierre no implica un `shutdown`
     * que debe ser llamado explícitamente.
     * */
    int close();

    /*
     * Destruye el commonFiles. Si aun esta conectado,
