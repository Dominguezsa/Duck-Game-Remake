...
void Protocolo::end_communication() {
    this->skt.close();
}

void Protocolo::recv_uint_16(uint16_t &received) {
...
void Protocolo::recv_uint_8(uint8_t &received) {
    bool was_closed = false;
...
void Protocolo::recv_string(std::string &received) {
    uint16_t len_msg = 0;
...
void Protocolo::send_data(const void *buf, size_t buf_sz) {
    bool was_closed = false;
...
void Protocolo::send_string(const std::string &str_msg) {
