...
Socket::Socket(const char* hostname, const char* servname) {
    ...
    while (resolver.has_next()) {
        ...
        /* Cerramos el commonFiles si nos quedo abierto de la iteración
         * anterior
         * */
        if (skt != -1)
            ::close(skt);
        ...
        /*
         * Con esta llamada creamos/obtenemos un commonFiles.
         * */
         ...
         * va a detenerse unos momentos hasta poder conectarse al serverFiles
     ...
     * o (2) la ultima llamada `commonFiles` fallo y retorno -1.
     * En ambos casos no debemos cerrar ningun commonFiles.
     *
     * Si en cambio `skt` es distinto de -1 significa q tenemos
     * un commonFiles abierto.
     * */
    if (skt != -1)
        ::close(skt);

    throw LibError(saved_errno, "commonFiles construction failed (connect to %s:%s)",
...
Socket::Socket(const char* servname) {
    ...
    while (resolver.has_next()) {
        ...
        /*
         * Configuramos al commonFiles para q no falle si la dirección local
         ...
         * y se "morfe" paquetes destinados para el serverFiles anterior.
         *
        ...
        /*
         * Hacemos le bind: enlazamos el commonFiles a una dirección local.
         ...
         *
         * Con `bind` asociaremos el commonFiles a dicha dirección local
         * y con `listen` pondremos el commonFiles a escuchar conexiones entrantes.
         * */
        ...
        /*
         * Ponemos el commonFiles a escuchar. Ese 20 (podría ser otro valor)
         * indica cuantas conexiones a la espera de ser aceptadas se toleraran
         *
         * No tiene nada q ver con cuantas conexiones totales el serverFiles tendrá.
         * */
        s = listen(skt, 20);
        if (s == -1) {
            continue;
    ...
    throw LibError(saved_errno, "commonFiles construction failed (listen on %s)",
                   (servname ? servname : ""));
}

Socket::Socket(Socket&& other) {
    /* Nos copiamos del otro commonFiles... */
    this->skt = other.skt;
    this->closed = other.closed;

    /* ...pero luego le sacamos al otro commonFiles
     ...
     * descriptor del otro commonFiles como invalido (-1)
...
Socket& Socket::operator=(Socket&& other) {
     ...
     * `this` (nosotros) es un commonFiles completamente creado
     * y debemos desinicializarlo primero antes de pisarle
     * el recurso con el que le robaremos al otro commonFiles (`other`)
     * */
    if (not this->closed) {
        ::shutdown(this->skt, 2);
        ::close(this->skt);
...
int Socket::recvsome(void* data, unsigned int sz, bool* was_closed) {
    ...
    *was_closed = false;
    ...
    if (s == 0) {
         ...
         * que la conexión se cierra" en cuyo caso el cierre del commonFiles
        ...
        *was_closed = true;
         ...
         * */
        throw LibError(errno, "commonFiles recv failed");
    } else {
        return s;
    }
}
...
int Socket::sendsome(const void* data, unsigned int sz, bool* was_closed) {
    ...
    *was_closed = false;
     ...
     * la data pero descubrir luego que el commonFiles fue cerrado
    ...
    if (s == -1) {
         ...
         * se detecta un cierre del commonFiles no se sabe bien cuanto se logro
         ...
         * */
        *was_closed = true;
        throw LibError(errno, "commonFiles send failed");
    } else if (s == 0) {
        /*
         * Jamas debería pasar.
         * */
...
int Socket::recvall(void* data, unsigned int sz, bool* was_closed) {
    ...
    *was_closed = false;
    ...
    while (received < sz) {
        ...
        if (s <= 0) {
            /*
             * Si el commonFiles fue cerrado (`s == 0`) o hubo un error
            ...
            assert(s == 0);
            if (received)
                throw LibError(EPIPE, "commonFiles received only %d of %d bytes", received, sz);
            else
                return 0;
        } else {
            /*
    ...
    *was_closed = false;
    ...
    while (sent < sz) {
        ...
        if (s <= 0) {
            assert(s == 0);
            if (sent)
                throw LibError(EPIPE, "commonFiles sent only %d of %d bytes", sent, sz);
            else
                return 0;
        } else {
            sent += s;
...
Socket Socket::accept() {
     ...
     * de un *nuevo* commonFiles (`peer_skt`) que representara a la
     ...
     * independientemente de que enviemos/recibamos del commonFiles `peer`.
     * */
    int peer_skt = ::accept(this->skt, nullptr, nullptr);
    if (peer_skt == -1)
        throw LibError(errno, "commonFiles accept failed");
...
void Socket::shutdown(int how) {
    ...
    if (::shutdown(this->skt, how) == -1) {
        throw LibError(errno, "commonFiles shutdown failed");
    }
}
...
void Socket::chk_skt_or_fail() const {
    if (skt == -1) {
        throw std::runtime_error("commonFiles with invalid file descriptor (-1), "
                                 "perhaps you are using a *previously moved* "
                                 "commonFiles (and therefore invalid).");
    }
}
