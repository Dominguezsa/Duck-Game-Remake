...
Tile::Tile(int row, int col, uint8_t &id, QGraphicsItem* parent)
    : QObject(), QGraphicsRectItem(parent), imagePixmap(), row(row), col(col)
    , image_id(id) {
...
void Tile::paint(QPainter* painter, [[maybe_unused]] const QStyleOptionGraphicsItem* option,
    ...
    QRectF innerRect = boundingRect().adjusted(TILE_BORDER_WIDTH, TILE_BORDER_WIDTH, -TILE_BORDER_WIDTH, -TILE_BORDER_WIDTH);
    ...
    if (!imagePixmap.isNull()) {
        ...
        QPixmap scaledPixmap = imagePixmap.scaled(innerRect.size().toSize(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
        painter->drawPixmap(innerRect.topLeft(), scaledPixmap); // Dibuja la imagen dentro del área ajustada
    }
}
